...
function confirmarSubstituicao() {
    ...
    return _enfileirarAcaoAgenda(() => _confirmarSubstituicaoInterno(ag, pac));
}

async function _confirmarSubstituicaoInterno(ag, pac) {
...
// a Agenda Online tem window.agendaOnlineBridge (página remota, preload
...
async function obterTokenServidorAgenda() {
    ...
    try {
        ...
        } else if (window.agendaOnlineBridge && typeof window.agendaOnlineBridge.obterTokenServidor === 'function') {
            _tokenServidorAgendaCache = await window.agendaOnlineBridge.obterTokenServidor();
        }
    } catch(e) {
