...
function iniciarComandoVozAgenda() {
    ...
    if (fab) fab.classList.add('ouvindo');
    toast('🎙️ Ouvindo... diga o comando completo', 4000);

    rec.onresult = (e) => {
        const texto = e.results[0][0].transcript;
        processarComandoVoz(texto);
    };
    rec.onerror = () => toast('Não consegui entender. Tente novamente.');
    rec.onend   = () => { if (fab) fab.classList.remove('ouvindo'); };

    try { rec.start(); } catch (e) { if (fab) fab.classList.remove('ouvindo'); }
}
...
function iniciarComandoVozNomeModal() {
    ...
    rec.onresult = (e) => {
        const texto = e.results[0][0].transcript;
        _aplicarNomeNoModalAdd(texto);
    };
    rec.onerror = () => toast('Não entendi o nome. Tente novamente.');
    rec.onend   = () => { if (btn) btn.classList.remove('ouvindo'); };

    try { rec.start(); } catch (e) { if (btn) btn.classList.remove('ouvindo'); }
}
...
function processarComandoVoz(textoOriginal) {
    ...
    if (acao === 'incluir_paciente') {
        ...
        toast(`Complete o cadastro de "${nome}" e salve.`, 4000);
        return;
    }
...
function _iniciarAgendamentoPorVoz(paciente, dataInfo, horaInfo) {
    ...
    toast(`🎙️ Entendi: ${paciente.nome}, ${horaLabel(horaInfo.hora)}. Confira e toque em Salvar.`, 4500);
}
