...
// ── Agenda Rápida (chamada vinda da Home do app desktop) ──
// Lê ?acao=agendar|substituir|cancelar|link na URL e guia o usuário
// direto para a ação certa, sem precisar navegar pela agenda inteira.
function processarAcaoRapidaUrl() {
    ...
    if (acao === 'agendar') {
        toast('Toque em um horário livre na grade para agendar.', 3500);
        return;
    }
    if (acao === 'substituir') {
        toast('Toque em uma consulta existente e escolha "Substituir".', 3500);
        return;
    }
    if (acao === 'cancelar') {
        toast('Toque em uma consulta existente e escolha "Cancelar".', 3500);
        return;
    }
}
