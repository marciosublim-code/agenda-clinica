...
//    inteiras na área visível, sem precisar rolar, em telas largas
//    (PC/tablet). Em celular (media queries com altura fixa), essa
//    variável é ignorada pelo CSS. ──
const GA_LARGURA_MIN_AUTOFIT = 481; // acima do último breakpoint mobile (480px)
function ajustarAlturaGradeAgenda() {
    if (window.innerWidth < GA_LARGURA_MIN_AUTOFIT) {
        document.documentElement.style.removeProperty('--ga-row-h');
        return;
    }
    ...
    const MIN_LINHA = 26; // abaixo disso os agendamentos ficam ilegíveis — aí volta a rolar
    let alturaLinha = Math.floor((alturaDisponivel - alturaHeader) / numLinhas);
    if (alturaLinha < MIN_LINHA) {
        document.documentElement.style.removeProperty('--ga-row-h');
        return;
    }
...
window.addEventListener('resize', () => {
    ...
    _gaResizeTimer = setTimeout(ajustarAlturaGradeAgenda, 120);
});
...
async function confirmarSubstituicao() {
    ...
    const sincronizouOk = await salvarAgendamentos_ls();
    ...
    toast(sincronizouOk
        ? `Paciente substituído por ${pac.nome}!`
