...
function abrirModalDetalhe(id) {
    ...
    // Botão Substituir Paciente
    let btnSub = $('btn-substituir-paciente');
    if (!btnSub) {
        btnSub = document.createElement('button');
        btnSub.id = 'btn-substituir-paciente';
        btnSub.className = 'btn-sec';
        btnSub.style.cssText = 'margin-top:.5rem;width:100%;';
        btnSub.innerHTML = '<i class="fa-solid fa-user-pen"></i> Substituir Paciente';
        const footer = document.querySelector('#modal-detalhe .modal-footer') || $('modal-detalhe-corpo').parentElement;
        if (footer) footer.insertBefore(btnSub, footer.querySelector('button'));
    }
    btnSub.onclick = () => abrirModalSubstituir();
...
// ══════════════════════════════════════════════════════
// SUBSTITUIR PACIENTE NO MESMO HORÁRIO
// ══════════════════════════════════════════════════════
...
function abrirModalSubstituir() {
    ...
    let modal = $('modal-substituir');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'modal-substituir';
        modal.className = 'modal-overlay';
        modal.style.display = 'none';
        modal.innerHTML = `
            <div class="modal-card">
                <div class="modal-header">
                    <span id="modal-sub-titulo"></span>
                    <button onclick="fecharModal('modal-substituir')" class="modal-close">✕</button>
                </div>
                <div class="modal-body">
                    <p id="modal-sub-info" style="font-size:.85rem;color:var(--text2);margin-bottom:.75rem;"></p>
                    <label style="font-size:.8rem;font-weight:600;color:var(--text2);display:block;margin-bottom:.3rem;">Novo paciente</label>
                    <select id="sub-pac-sel" style="width:100%;padding:.6rem;border-radius:8px;border:1.5px solid var(--border);font-size:.9rem;background:var(--bg);color:var(--text);">
                        <option value="">— Selecione —</option>
                    </select>
                </div>
                <div class="modal-footer" style="display:flex;gap:.5rem;padding:.75rem 1rem;">
                    <button class="btn-sec" onclick="fecharModal('modal-substituir')" style="flex:1;">Cancelar</button>
                    <button class="btn-pri" onclick="confirmarSubstituicao()" style="flex:2;">
                        <i class="fa-solid fa-check"></i> Confirmar substituição
                    </button>
                </div>
            </div>
        `;
    ...
    const titulo = $('modal-sub-titulo');
    if (titulo) titulo.innerHTML = '<i class="fa-solid fa-user-pen"></i> Substituir Paciente';
    const info = $('modal-sub-info');
    if (info) info.textContent = `Horário: ${DIAS_FULL[d.getDay()]}, ${d.getDate()} de ${MESES_ABR[d.getMonth()]} às ${horaLabel(ag.hora)}`;

    // Popula select com pacientes (exceto o atual)
...
async function confirmarSubstituicao() {
    ...
    // Salva no Drive com o antigo marcado como cancelado + novo confirmado
    const listaAtual = [...S.agendamentos];
    S.agendamentos = [...listaAtual, { ...ag, status: 'cancelado' }];
    await salvarAlteracoesNoDrive();
    S.agendamentos = listaAtual;
}
