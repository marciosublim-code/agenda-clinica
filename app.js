...
async function renderizarAgenda() {
    ...
    HORAS_CHEIAS.forEach(h => {
        ...
        dias.forEach(d => {
            ...
            cel.className = 'ga-celula ga-celula-dupla';

            SUB_OFFSETS.forEach(offset => {
                const hSlot = h + offset;
                const ag    = S.agendamentos.find(a => a.data === iso && parseFloat(a.hora) === hSlot);
                const sub   = document.createElement('div');
                sub.className = 'ga-sub-slot ' + (ag ? 'agendado' : 'livre');
                ...
                if (ag) {
                    ...
                    sub.onclick = () => abrirModalDetalhe(ag.id);
                } else {
                    const rotulo = document.createElement('span');
                    rotulo.className = 'ga-sub-label';
                    rotulo.textContent = offset === 0 ? 'cheia' : 'meia';
                    sub.appendChild(rotulo);
                    sub.title = horaLabel(hSlot);
                    sub.onclick = () => abrirModalAdd(iso, hSlot);
                }
                cel.appendChild(sub);
            });
