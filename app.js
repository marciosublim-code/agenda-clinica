...
async function pinEnter() {
    ...
    if (S.pin === pinCorreto) {
        ...
        // ── Após PIN correto: decide próximo passo ──────────────────────────
        if (tokenValido()) {
            // Já tem sessão Drive válida → entra direto na agenda
            await _entrarNaAgendaAposDrive();
        } else if (!navigator.onLine) {
            // Sem internet → entra na agenda com banner de aviso offline
            await _entrarNaAgendaAposDrive(true);
        } else {
            // Tem internet mas sem Drive → mostra tela intermediária de login Drive
            _mostrarTelaLoginDrive();
        }
    } else {
...
// ── Entra na agenda após Drive (ou sem Drive se offline) ─────────────────────
async function _entrarNaAgendaAposDrive(offline = false) {
    irTela('tela-agenda');
    atualizarStatusDrive();
    ...
    if (offline) {
        ...
        window.addEventListener('online', async () => {
            ...
            if (!tokenValido()) {
                setTimeout(() => _mostrarTelaLoginDrive(), 800);
            }
        }, { once: true });
    } else {
...
// ── Overlay de login Drive (entre PIN e agenda) ───────────────────────────────
function _mostrarTelaLoginDrive() {
    let overlay = document.getElementById('overlay-drive-login');
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'overlay-drive-login';
        ...
        overlay.innerHTML = `
            <div style="background:#fff;border-radius:1.5rem;padding:2.5rem 2rem;max-width:340px;width:90%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
                <div style="font-size:3rem;margin-bottom:0.5rem;">&#9729;&#65039;</div>
                <h2 style="margin:0 0 0.5rem;font-size:1.3rem;color:#1e293b;">Conectar ao Google Drive</h2>
                <p style="color:#64748b;font-size:0.92rem;margin:0 0 1.5rem;line-height:1.5;">
                    Faça login para sincronizar seus agendamentos automaticamente entre o celular e o sistema local.
                </p>
                <button onclick="_conectarDriveAposPin()" style="width:100%;padding:0.85rem;border-radius:0.75rem;border:none;background:#4285f4;color:#fff;font-size:1rem;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:0.6rem;margin-bottom:0.75rem;">
                    <i class='fa-brands fa-google'></i> Entrar com Google
                </button>
                <button onclick="_pularLoginDrive()" style="width:100%;padding:0.75rem;border-radius:0.75rem;border:1.5px solid #e2e8f0;background:transparent;color:#64748b;font-size:0.9rem;cursor:pointer;">
                    Entrar sem Drive por agora
                </button>
            </div>`;
...
function _conectarDriveAposPin() {
    lsSet('agenda_oauth_origem', 'agenda');
    lsSet('agenda_oauth_estava_logado', true);
    // Faz o redirect diretamente para não chamar conectarGoogleDriveMobile(),
    // que sobrescreveria agenda_oauth_origem com 'login' (linha 55).
    const redirectUri = encodeURIComponent(window.location.origin + window.location.pathname);
    const url = `https://accounts.google.com/o/oauth2/v2/auth`
        + `?client_id=${GOOGLE_CLIENT_ID}`
        + `&redirect_uri=${redirectUri}`
...
function _pularLoginDrive() {
    const overlay = document.getElementById('overlay-drive-login');
    if (overlay) overlay.style.display = 'none';
    _mostrarBannerOffline('sem-drive');
    // Entra na agenda normalmente (sem polling Drive)
    irTela('tela-agenda');
    atualizarStatusDrive();
    renderizarAgenda();
}

// ── Banner de aviso na base da tela ──────────────────────────────────────────
function _mostrarBannerOffline(tipo) {
    tipo = tipo || 'offline';
    let banner = document.getElementById('banner-offline-aviso');
    if (!banner) {
        banner = document.createElement('div');
        banner.id = 'banner-offline-aviso';
    ...
    const msg = tipo === 'sem-drive'
        ? '&#9888;&#65039; Drive não conectado — agendamentos não serão sincronizados.'
        : '&#128225; Sem internet — ao voltar a conexão, faça login no Drive para sincronizar.';
    banner.innerHTML = '<span>' + msg + '</span>'
        + '<button onclick="_esconderBannerOffline()" style="background:transparent;border:none;color:#fff;font-size:1.1rem;cursor:pointer;padding:0 0.25rem;">&#10005;</button>';
    banner.style.display = 'flex';
}

function _esconderBannerOffline() {
    var banner = document.getElementById('banner-offline-aviso');
    if (banner) banner.style.display = 'none';
}
function logout() {
    S.adminPin = null;
...
async function cancelarAgendamento() {
    ...
    // Salva no Drive com o agendamento marcado como cancelado
    // para o desktop importar o status e excluir do banco
    const listaOriginal = S.agendamentos;
    S.agendamentos = [...listaOriginal, { ...ag, status: 'cancelado' }];
    await salvarAlteracoesNoDrive();
    S.agendamentos = listaOriginal;
}
