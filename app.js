...
function abrirImagemDia(src) {
  ...
  lb.id = "pp-lightbox";
  lb.style.cssText = ` position:fixed;inset:0;z-index:9999; background:rgba(61,37,53,0.85); display:flex;align-items:center;justify-content:center; backdrop-filter:blur(8px); animation:fadeInLb .2s ease-out; cursor:zoom-out; `;
  lb.onclick = () => lb.remove();
  const img = document.createElement("img");
  img.src = src;
  img.style.cssText = ` max-width:90vw;max-height:90vh; border-radius:16px; box-shadow:0 16px 60px rgba(0,0,0,0.5); object-fit:contain; animation:zoomInLb .25s ease-out; `;
  lb.appendChild(img);
  document.body.appendChild(lb);
}

/* ============================================================ APP.JS — Agenda Clínica PWA Armazenamento: Google Drive (fonte principal) + IndexedDB (offline) Tailscale/servidor local: REMOVIDO ============================================================ */
...
const GOOGLE_CLIENT_ID =
  "235501323072-0eq60qvktqbsalrp1htt3uqkf26fkq5s.apps.googleusercontent.com";
...
function atualizarStatusDrive() {
  ...
  if (btnTexto)
    btnTexto.textContent = conectado
  ...
  if (btnLogin) {
    if (conectado) {
      btnLogin.innerHTML =
        '<i class="fa-brands fa-google"></i> ☁️ Agendamentos salvos automaticamente';
      btnLogin.style.background = "#e8f5e9";
      btnLogin.style.color = "#2e7d32";
    } else {
      btnLogin.innerHTML =
        '<i class="fa-brands fa-google"></i> Clique aqui para salvar seus agendamentos';
...
function agendarRenovacaoToken() {
  ...
  console.log(
    "[Drive] Renovação do token agendada em",
    Math.round(renovarEm / 60000),
    "min"
  );
...
function renovarTokenSilencioso() {
  ...
  iframe.onload = () => {
    try {
      ...
      console.warn(
        "[Drive] Renovação silenciosa falhou, usuário precisará reconectar."
      );
    }
  ...
  try {
    ...
    if (!result.files || result.files.length === 0) {
      ...
      console.warn(
        "[Drive] Arquivo",
        DRIVE_FILE_NAME,
        "não encontrado no Drive."
      );
      if (!silencioso)
        toast(
          "☁️ Drive conectado! Nenhum backup encontrado ainda — os dados serão criados no próximo salvamento."
        );
      return false;
    }
    ...
    if (
      silencioso &&
      _driveUltimaModificacao &&
      modDrive <= _driveUltimaModificacao
    ) {
      return false;
    }
    ...
    console.log(
      "[Drive] Conteúdo baixado — pacientes:",
      pacientes.length,
      "| agendamentos:",
      agendamentos.length,
      "| tokens:",
      Object.keys(tokensObj).length
    );
    ...
    if (config) lsSet("agenda_config", config);
...
async function salvarAlteracoesNoDrive(listaOverride = null) {
  ...
  if (!tokenValido()) {
    ...
    if (listaOverride) {
      const canceladosPendentes = listaOverride.filter(
    ...
    console.warn(
      "[Drive] Sem token válido — dados salvos localmente, sync pendente."
    );
    return;
  }
  ...
  const lista = [
    ...listaBase,
    ...canceladosPendentes.filter((c) => !listaBase.find((a) => a.id === c.id)),
  ];
  const payload = {
  ...
  try {
    ...
    if (listaOverride) {
      const _canceladosFalha = listaOverride.filter(
        (a) => a.status === "cancelado"
      );
...
function resolverDuplicatasAgendamentos() {
  const ativos = S.agendamentos.filter(
    (a) => (a.status || "confirmado") !== "cancelado"
  );
  const grupos = {};
  ativos.forEach((a) => {
  ...
  if (houveDuplicata) {
    ...
    S.agendamentos = S.agendamentos.filter(
      (a) => !idsParaCancelar.includes(a.id)
    );
...
function iniciarPollingDrive() {
  ...
  _drivePollingTimer = setInterval(async () => {
    if (!tokenValido()) {
      clearInterval(_drivePollingTimer);
      return;
    }
    await baixarBackupDrive(true);
  }, DRIVE_POLL_MS);
  console.log(
    `[Drive] Polling ativo — verificando a cada ${DRIVE_POLL_MS / 1000}s`
  );
}

function pararPollingDrive() {
  if (_drivePollingTimer) {
    clearInterval(_drivePollingTimer);
...
const SUB_OFFSETS = Array.from(
  { length: 60 / INTERVALO_MIN },
  (_, i) => i * (INTERVALO_MIN / 60)
); // [0, 0.5]
...
function salvarAgendamentos_ls() {
  ...
  // Atualiza IndexedDB também
  idbClear("agendamentos")
    .then(() => {
      S.agendamentos.forEach((ag) =>
        idbPut("agendamentos", ag).catch(() => {})
      );
    })
    .catch(() => {});
}
...
function irTela(id) {
  ...
  if (id === "tela-login") {
    ...
    if (btnDrive) {
      if (tokenValido()) {
        btnDrive.innerHTML =
          '<i class="fa-brands fa-google"></i> Google Drive sincronizado ✓';
        btnDrive.style.background = "#e8f5e9";
        btnDrive.style.color = "#2e7d32";
        btnDrive.style.border = "1.5px solid #a5d6a7";
      } else {
        btnDrive.innerHTML =
          '<i class="fa-brands fa-google"></i> Sincronizar via Google Drive';
...
function atualizarPinDisplay() {
  ...
  spans.forEach((s, i) => {
    s.className = i < S.pin.length ? "filled" : "";
  });
  const inp =
    document.querySelector('input[type="password"]') || $("login-pin");
...
async function pinEnter() {
  ...
  if (S.pin === pinCorreto) {
    ...
    if (msgErro) {
      ...
      msgErro.textContent = "PIN incorreto.";
    }
    S.pin = "";
    atualizarPinDisplay();
    setTimeout(() => {
...
function mostrarGateDrive(estado) {
  ...
  if (estado === "sincronizando") {
    gate.innerHTML = ` <div class="drive-gate-card"> <div class="drive-gate-icon spin"><i class="fa-brands fa-google-drive"></i></div> <h3>Sincronizando...</h3> <p>Baixando dados do Google Drive</p> </div>`;
    gate.style.display = "flex";
    return;
  }

  if (estado === "offline") {
    gate.innerHTML = ` <div class="drive-gate-card"> <div class="drive-gate-icon offline"><i class="fa-solid fa-wifi-slash"></i></div> <h3>Sem conexão</h3> <p>Você está offline. Os agendamentos serão salvos no celular e enviados ao sistema quando conectar ao Google Drive.</p> <button class="btn-pri" style="width:100%;margin-top:1rem;" onclick="fecharGateDrive();abrirAgenda();"> <i class="fa-solid fa-arrow-right"></i> Entrar mesmo assim </button> </div>`;
    gate.style.display = "flex";
    lsSet("agenda_sync_pendente", true);
    return;
  }
  ...
  if (estado === "desconectado") {
    gate.innerHTML = ` <div class="drive-gate-card"> <div class="drive-gate-icon"><i class="fa-brands fa-google-drive"></i></div> <h3>Conecte ao Google Drive</h3> <p>Para sincronizar seus agendamentos com o sistema, autorize o acesso ao Google Drive.</p> <button class="btn-pri" style="width:100%;margin-top:1rem;" onclick="fecharGateDrive();conectarDriveAgenda();"> <i class="fa-brands fa-google"></i> Autorizar Google Drive </button> <button class="btn-sec" style="width:100%;margin-top:.5rem;" onclick="fecharGateDrive();abrirAgenda();lsSet('agenda_sync_pendente',true);"> Continuar sem Drive <span style="display:block;font-size:.72rem;opacity:.7;margin-top:.2rem;">Dados ficam só no celular</span> </button> </div>`;
    gate.style.display = "flex";
    return;
  }
}
...
async function carregarTudo() {
  ...
  await carregarPacientes_ls();
  await carregarAgendamentos_ls();
  if ($("menu-clinica-nome"))
    $("menu-clinica-nome").textContent = S.config.nome_clinica;
  if ($("cfg-nome-clinica"))
    $("cfg-nome-clinica").value = S.config.nome_clinica;
...
async function renderizarAgenda() {
  ...
  dias.forEach((d) => {
    ...
    const imgIdx =
      (d.getDay() + S.semanaOffset * 7 + Math.floor(d.getDate() / 7)) %
      (imgs.length || 1);
    const imgSrc = imgs.length ? imgs[imgIdx] : "";
    const imgTag = imgSrc
      ? `<img src="${imgSrc}" class="ga-dia-img" alt="✿" onclick="event.stopPropagation();abrirImagemDia('${imgSrc}')" onerror="this.style.display='none'" title="Clique para ampliar" style="cursor:zoom-in;">`
      : "✿";
  ...
  HORAS_CHEIAS.forEach((h) => {
    ...
    dias.forEach((d) => {
      ...
      const slotsHora = SUB_OFFSETS.map((offset) => {
        ...
        return {
          hSlot,
          ag: S.agendamentos.find(
            (a) => a.data === iso && parseFloat(a.hora) === hSlot
          ),
        };
      });
      ...
      if (!ocupados.length) {
        ...
        ocupados.forEach(({ ag, hSlot }) => {
          ...
          const nomeMini = (ag.nome_paciente || ag.paciente || "").split(
            " "
          )[0];
...
function renderizarListaDia() {
  ...
  if (headerEl)
    headerEl.textContent = `${DIAS_FULL[d.getDay()]}, ${d.getDate()} de ${ MESES_ABR[d.getMonth()] }`;

  const lista = $("dia-lista");
  if (!lista) return;
  const agsDia = S.agendamentos
    .filter((a) => a.data === iso)
  ...
  agsDia.forEach((ag) => {
    ...
    const cod = ag.codigo_paciente
      ? `#${String(ag.codigo_paciente).padStart(3, "0")}`
      : "";
    const nome = ag.nome_paciente || ag.paciente || "Paciente";
    const st = ag.status || "confirmado";
    div.innerHTML = ` <div class="ag-hora">${horaLabel(ag.hora)}</div> <div class="ag-info"> <div class="ag-nome">${nome}</div> ${cod ? `<div class="ag-cod">${cod}</div>` : ""} </div> <span class="ag-badge ${st}">${ st === "confirmado" ? "Confirmado" : "Aguardando" }</span> `;
...
function irHoje() {
  S.semanaOffset = 0;
  S.diaSelecionado = isoDate(new Date());
  renderizarAgenda();
}
...
function selecionarPeriodoImprimir(tipo) {
  S.impPeriodo = tipo;
  document
    .querySelectorAll("#imp-periodo-grupo .toggle-horario-btn")
    .forEach((b) => {
...
function gerarImpressaoAgenda() {
  ...
  const linhas = lista.length
    ? lista
        .map((ag) => {
          ...
          return `<tr> ${ mostrarColunaData ? `<td>${_formatarDataCurtaImp(ag.data)}</td>` : "" } <td>${horaLabel(ag.hora)}</td> <td>${cod}${nome}</td> <td>${stLabel}</td> <td>${ag.obs || ""}</td> </tr>`;
        })
        .join("")
    : `<tr><td colspan="${ mostrarColunaData ? 5 : 4 }" style="text-align:center;color:#666;">Nenhuma consulta neste período</td></tr>`;

  const cabecalhoCols =
    (mostrarColunaData ? "<th>Data</th>" : "") +
  ...
  if (!area) return;
  area.innerHTML = ` <div class="imp-cabecalho"> <h1>${S.config.nome_clinica || "Agenda Clínica"}</h1> <h2>${tituloPeriodo}</h2> </div> <table class="imp-tabela"> <thead><tr>${cabecalhoCols}</tr></thead> <tbody>${linhas}</tbody> </table> <p class="imp-rodape">Gerado em ${geradoEm} — ${lista.length} consulta${ lista.length === 1 ? "" : "s" }</p> `;
...
function abrirModalDetalhe(id) {
  ...
  const st = ag.status || "confirmado";
  const cod = ag.codigo_paciente
    ? `<span class="pac-codigo">#${String(ag.codigo_paciente).padStart( 3, "0" )}</span>`
    : "";
  $("modal-detalhe-corpo").innerHTML = ` <div class="detalhe-row"><span class="dr-label">Paciente</span><span class="dr-val">${cod} ${nome}</span></div> <div class="detalhe-row"><span class="dr-label">Data</span><span class="dr-val">${ DIAS_FULL[d.getDay()] }, ${d.getDate()} de ${MESES_ABR[d.getMonth()]}</span></div> <div class="detalhe-row"><span class="dr-label">Horário</span><span class="dr-val">${horaLabel( ag.hora )} – ${horaLabel(parseFloat(ag.hora) + 1)}</span></div> <div class="detalhe-row"><span class="dr-label">Status</span><span class="dr-val"><span class="ag-badge ${st}">${ st === "confirmado" ? "✅ Confirmado" : "⏳ Aguardando" }</span></span></div> ${ ag.obs ? `<div class="detalhe-row"><span class="dr-label">Obs</span><span class="dr-val">${ag.obs}</span></div>` : "" } `;
...
function abrirModalSubstituir() {
  ...
  if (info)
    info.textContent = `${DIAS_FULL[d.getDay()]}, ${d.getDate()} de ${ MESES_ABR[d.getMonth()] } — ${horaLabel(ag.hora)}`;
...
async function confirmarSubstituicao() {
  ...
  if (!ag) {
    toast("Erro: agendamento não encontrado.");
    return;
  }
  ...
  if (!_canceladosSub.find((c) => c.id === ag.id)) {
    lsSet("agenda_cancelados_pendentes", [
      ..._canceladosSub,
      { ...ag, status: "cancelado" },
    ]);
    lsSet("agenda_sync_pendente", true);
...
async function salvarAgendamentoManual() {
  ...
  const qtd =
    recorrencia !== "nao" ? parseInt($("add-recorr-qtd")?.value || 1) : 1;
...
function atualizarPreviewRecorrencia() {
  ...
  if (datas.length > 5)
    linhas.push(
      `<span style="color:#c4506d;font-weight:600;">+ ${ datas.length - 5 } mais...</span>`
    );
...
function onModalPacienteChange() {
  ...
  const cod = pac.codigo
    ? `<span class="pac-codigo">#${String(pac.codigo).padStart(3, "0")}</span>`
    : "";
  info.style.display = "flex";
  info.innerHTML = ` ${cod} <span class="pac-info-nome">${pac.nome}</span> ${ pac.telefone ? `<span class="pac-info-tel"><i class="fa-solid fa-phone"></i> ${pac.telefone}</span>` : "" } `;
}
...
function renderizarGradeModal() {
  ...
  dias.forEach((d) => {
    ...
    HORAS.forEach((h) => {
      const chave = `${iso}_${h}`;
      const ocupado = S.agendamentos.some(
        (a) => a.data === iso && parseFloat(a.hora) === h
      );
...
async function gerarLink() {
  ...
  Object.entries(S.slotStates).forEach(([chave, estado]) => {
    ...
    if (estado === "indisponivel")
      bloqueados.push({ data, hora: parseFloat(hora) });
  });
  if (!slots.length) {
    toast("Marque ao menos um horário disponível (✓).");
    return;
  }

  const token =
    "tk_" + Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
  const dadosToken = {
    pacienteId: pac.id,
    nomePaciente: pac.nome,
    slots,
  ...
  if (lv) {
    ...
    if (!linkEl) {
      ...
      linkEl.style.cssText =
        "display:block;word-break:break-all;color:var(--accent2);font-size:.8rem;margin-top:.5rem;text-decoration:underline;cursor:pointer;";
...
function copiarUrlAgenda() {
  ...
  if (navigator.clipboard && window.isSecureContext) {
    navigator.clipboard
      .writeText(texto)
      .then(() => toast("📋 Endereço copiado! Cole no navegador do celular."));
    return;
  }
  ...
  try {
    document.execCommand("copy");
    toast("📋 Endereço copiado! Cole no navegador do celular.");
  } catch (e) {
    toast("Copie manualmente: " + texto, 4000);
  }
...
async function abrirModalFixos() {
  ...
  corpo.innerHTML =
    '<p style="color:var(--text2);padding:1rem;">Gerando links...</p>';
  ...
  if (!pacFixos.length) {
    corpo.innerHTML =
      '<p style="color:var(--text2);padding:1rem;text-align:center;">Nenhum paciente com horário fixo cadastrado.</p>';
    return;
  }
  ...
  pacFixos.forEach((p) => {
    const hf =
      typeof p.horario_fixo === "string"
        ? JSON.parse(p.horario_fixo)
        : p.horario_fixo;
    ...
    div.innerHTML = ` <div class="fixo-item-info"> <div class="fixo-item-nome">${p.nome}</div> <div class="fixo-item-horario">${ DIAS_FULL[hf.diaSemana] }, ${dataFixa.getDate()} de ${ MESES_ABR[dataFixa.getMonth()] } — ${horaLabel(hf.hora)}</div> </div> <a class="fixo-item-link" href="https://wa.me/${( p.telefone || "" ).replace(/\D/g, "")}?text=${encodeURIComponent( `Olá, ${p.nome}! Confirme sua consulta:\n\n${link}` )}" target="_blank"> <i class="fa-brands fa-whatsapp"></i> Enviar </a> `;
...
function renderizarListaPacientes(filtro = "") {
  ...
  arr.forEach((p) => {
    ...
    div.innerHTML = ` <div class="pac-avatar">${(p.nome || "?")[0].toUpperCase()}</div> <div class="pac-item-info"> <div class="pac-item-nome">${p.nome}</div> <div class="pac-item-sub" style="display:flex;gap:.4rem;flex-wrap:wrap;margin-top:.25rem;"> ${cod ? `<span class="pac-codigo-tag">${cod}</span>` : ""} ${ p.telefone ? `<span style="font-size:.75rem;color:var(--text2);">${p.telefone}</span>` : "" } ${ hf ? `<span class="pac-fixo-tag"><i class="fa-solid fa-rotate"></i> Fixo ${ DIAS_ABR[hf.diaSemana] } ${horaLabel(hf.hora)}</span>` : "" } </div> </div> <button onclick="editarPaciente('${ p.id }')" style="background:none;border:none;color:var(--text2);cursor:pointer;padding:.5rem;"><i class="fa-solid fa-pen"></i></button> `;
    lista.appendChild(div);
  });
}
...
function toggleHorarioFixo() {
  ...
  if (fixoOpts && check)
    fixoOpts.style.display = check.checked ? "grid" : "none";
}
...
async function salvarPaciente() {
  ...
  const hf = $("pac-fixo-check")?.checked
    ? {
        diaSemana: parseInt($("pac-fixo-dia").value),
        hora: parseFloat($("pac-fixo-hora").value),
      }
    : null;
  ...
  const dados = {
    ...
    horario_fixo: hf,
    codigo: editId
      ? S.pacientes.find((p) => String(p.id) === editId)?.codigo ||
...
function salvarConfig() {
  ...
  if (pin) {
    if (pin.length !== 4 || !/^\d{4}$/.test(pin)) {
      toast("PIN deve ter 4 dígitos.");
      return;
    }
    S.config.admin_pin = pin;
    S.adminPin = pin;
  }
  S.config.nome_clinica =
    ($("cfg-nome-clinica")?.value || "").trim() || "Agenda Clínica";
...
function alternarTemaAgenda() {
  ...
  const proximo =
    atual === "rosa" ? "azul" : atual === "azul" ? "verde" : "rosa";
...
async function iniciarTelaPaciente() {
  ...
  if (!cfg) {
    renderizarPacErro("Link inválido ou expirado.");
    return;
  }
  if (cfg.usado) {
    renderizarPacErro("Este link já foi utilizado.");
    return;
  }
  ...
  if (subtituloEl)
    subtituloEl.textContent = `Olá, ${cfg.nomePaciente}! Confirme seu horário.`;
  renderizarPacGrade();
}
...
function renderizarPacGrade() {
  ...
  const datas = [
    ...new Set([...slots, ...bloqueados].map((s) => s.data)),
  ].sort();
  ...
  datas.forEach((iso) => {
    ...
    const hdr = document.createElement("div");
    hdr.className =
      "pac-col-header" + (iso === isoDate(new Date()) ? " hoje" : "");
    ...
    horasDia.forEach((h) => {
      ...
      if (!isBloq) {
        btn.onclick = () => {
          document
            .querySelectorAll(".pac-slot.selecionado")
...
function renderizarPacConfirmacao() {
  ...
  corpo.innerHTML = ` <div class="pac-confirmacao"> <i class="fa-solid fa-calendar-check" style="font-size:2.5rem;color:var(--accent2);display:block;margin-bottom:1rem;"></i> <h3>Confirmar consulta?</h3> <p>Você está confirmando:</p> <div class="pac-confirm-slot">📅 ${data} às ${horaLabel( pacSlot.hora )}</div> <div class="pac-btns"> <button class="btn-sec" onclick="renderizarPacGrade()">Voltar</button> <button class="btn-pri" onclick="confirmarConsulta()"> <i class="fa-solid fa-check"></i> Confirmar </button> </div> </div> `;
}

async function confirmarConsulta() {
  if (!pacToken || !pacSlot) {
    renderizarPacErro("Nenhum horário selecionado.");
    return;
  }
  ...
  const jaOcupado = S.agendamentos.some(
    (a) =>
      a.data === pacSlot.data && parseFloat(a.hora) === parseFloat(pacSlot.hora)
  );
  if (jaOcupado) {
    renderizarPacErro(
  ...
  const corpo = $("pac-corpo");
  if (corpo)
    corpo.innerHTML = ` <div class="pac-sucesso" style="text-align:center;padding:2rem 1rem;"> <i class="fa-solid fa-circle-check" style="font-size:3.5rem;color:var(--success);display:block;margin-bottom:1rem;"></i> <h2>Consulta Confirmada!</h2> <p style="color:var(--text2);font-size:0.95rem;margin-bottom:1.5rem;">Seu horário foi reservado com sucesso.</p> <div style="background:var(--bg2);padding:1rem;border-radius:8px;text-align:left;border:1px solid var(--border);"> <div><strong>Nome:</strong> ${pacConfig.nomePaciente}</div> <div><strong>Data:</strong> ${pacSlot.data .split("-") .reverse() .join("/")}</div> <div><strong>Horário:</strong> ${horaLabel(pacSlot.hora)}</div> </div> </div> `;
}

function renderizarPacErro(msg) {
  const corpo = $("pac-corpo");
  if (corpo)
    corpo.innerHTML = ` <div class="pac-erro"> <i class="fa-solid fa-circle-exclamation" style="font-size:2.5rem;color:var(--danger);display:block;margin-bottom:1rem;"></i> <h2>Ops!</h2> <p>${ msg || "Ocorreu um erro. Tente novamente ou entre em contato com a clínica." }</p> </div> `;
}
...
function mostrarBannerInstalar() {
  ...
  let instrucao = "";
  if (isSafari && isIOS) {
    instrucao = ` <div class="pwa-instrucao-ios"> <p>No Safari, toque em <strong><i class="fa-solid fa-arrow-up-from-bracket"></i> Compartilhar</strong> na barra inferior e depois em <strong>"Adicionar à Tela de Início"</strong>.</p> </div>`;
  }
  const overlay = document.createElement("div");
  overlay.id = "pwa-install-overlay";
  overlay.className = "pwa-install-overlay";
  overlay.innerHTML = ` <div class="pwa-install-card"> <div class="pwa-icon"><i class="fa-solid fa-stethoscope"></i></div> <h3>Instalar Agenda Clínica</h3> <p>Deseja adicionar um ícone na ${ isMobile ? "tela inicial do celular" : "área de trabalho do computador" } para abrir o app com um clique?</p> ${instrucao} <div class="pwa-install-btns"> <button class="btn-pri" onclick="confirmarInstalarPWA()"> <i class="fa-solid fa-download"></i> Sim, instalar </button> <button class="btn-sec" onclick="recusarInstalarPWA()">Agora não</button> </div> </div> `;
...
async function confirmarInstalarPWA() {
  ...
  if (_pwaPrompt) {
    ...
    card.innerHTML = ` <div class="pwa-install-card"> <div class="pwa-icon" style="background:linear-gradient(135deg,#007aff,#0055cc);"> <i class="fa-brands fa-safari"></i> </div> <h3>Como instalar no iPhone/iPad</h3> <ol class="pwa-steps"> <li>Toque no botão <strong><i class="fa-solid fa-arrow-up-from-bracket"></i></strong> na barra inferior do Safari</li> <li>Role para baixo e toque em <strong>"Adicionar à Tela de Início"</strong></li> <li>Toque em <strong>"Adicionar"</strong> no canto superior direito</li> </ol> <div class="pwa-install-btns"> <button class="btn-pri" onclick="document.getElementById('pwa-ios-instrucao').remove()"> <i class="fa-solid fa-check"></i> Entendido </button> </div> </div> `;
    document.body.appendChild(card);
  } else {
    const card = document.createElement("div");
    card.id = "pwa-outros-instrucao";
    card.className = "pwa-install-overlay";
    card.innerHTML = ` <div class="pwa-install-card"> <div class="pwa-icon"><i class="fa-solid fa-globe"></i></div> <h3>Como instalar</h3> <ol class="pwa-steps"> <li>Clique nos <strong>três pontos ⋮</strong> no canto superior direito do navegador</li> <li>Procure por <strong>"Instalar aplicativo"</strong> ou <strong>"Adicionar à tela inicial"</strong></li> <li>Confirme a instalação</li> </ol> <div class="pwa-install-btns"> <button class="btn-pri" onclick="document.getElementById('pwa-outros-instrucao').remove()"> <i class="fa-solid fa-check"></i> Entendido </button> </div> </div> `;
    document.body.appendChild(card);
  }
}
