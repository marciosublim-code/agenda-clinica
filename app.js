...
function abrirAgenda() {
    ...
    renderizarAgenda();
    iniciarPollingDrive();
    processarAcaoRapidaUrl();
}

// ── Agenda Rápida ─────────────────────────────────────────
// Ativada quando a URL contém ?acao=agendar|substituir|cancelar|link
// (enviada pelo card "Agenda rápida" na Home do PsicoGestão).
function processarAcaoRapidaUrl() {
    const params = new URLSearchParams(location.search);
    const acao = params.get('acao');
    if (!acao || S._acaoRapidaProcessada) return;
    S._acaoRapidaProcessada = true;

    _aplicarModoAgendaRapida(acao);

    if (acao === 'agendar') {
        // Calcula hoje + próxima hora cheia dentro do range (7–19)
        const agora    = new Date();
        const hoje     = agora.toISOString().slice(0, 10); // "YYYY-MM-DD"
        const proxHora = Math.min(Math.max(7, agora.getHours() + 1), 19);
        abrirModalAdd(hoje, proxHora);
        return;
    }

    if (acao === 'link') {
        // Esconde a grade — o modal de link ocupa a tela toda
        const tela = $('tela-agenda');
        if (tela) tela.style.visibility = 'hidden';
        abrirModalGerarLink();
        return;
    }

    if (acao === 'substituir') {
        toast('Toque em uma consulta existente e escolha "Substituir".', 4000);
        return;
    }

    if (acao === 'cancelar') {
        toast('Toque em uma consulta existente e escolha "Cancelar".', 4000);
        return;
    }
}

// Injeta CSS que esconde controles irrelevantes para o modo de ação rápida.
function _aplicarModoAgendaRapida(acao) {
    document.body.classList.add('modo-agenda-rapida');
    const estilo = document.createElement('style');
    estilo.textContent = `
        body.modo-agenda-rapida .header-left button[onclick*="tela-menu"],
        body.modo-agenda-rapida #btn-drive-agenda,
        body.modo-agenda-rapida #btn-alternar-tema-agenda,
        body.modo-agenda-rapida .fab-fixos { display:none !important; }
        ${acao !== 'link'
            ? "body.modo-agenda-rapida .btn-novo[onclick*='abrirModalGerarLink']{display:none!important;}"
            : ''}
    `;
