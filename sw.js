...
self.addEventListener('fetch', event => {
    ...
    // API do Google Drive e Google APIs: sempre rede, sem cache nunca
        ...
        caches.match(event.request).then(cached => {
            const fetchPromise = fetch(event.request)
                .then(r => {
                    ...
                    // ✅ CORREÇÃO: clona ANTES de usar para evitar "body already used"
                    const rClone = r.clone();
                    const nomecache = url.origin === self.location.origin
                        ? CACHE_NOME : CACHE_EXTERNO;
                    caches.open(nomecache).then(c => c.put(event.request, rClone));
                    return r;
                })
                .catch(() => null);
